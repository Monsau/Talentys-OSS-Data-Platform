...
Bu kılavuz Airbyte, Dremio, dbt, Apache Superset, PostgreSQL, MinIO ve Elasticsearch dahil tüm platform bileşenlerinin yapılandırmasını kapsar. Doğru yapılandırma, hizmetler arasında optimum performansı, güvenliği ve entegrasyonu sağlar.
...
Projenin kökündeki `.env` dosyasını oluşturun veya düzenleyin:
...
**Asla hassas verileri işlemeyin:**
```bash
...
#### Mevcut Kaynakların Yapılandırması
...
### 5. Apache Süper Kümesi Yapılandırması

#### Uygulama Ayarları

**Dosya**: `config/superset_config.py`
...
### 7. Elasticsearch kurulumu

**Dosya**: `config/elasticsearch.yml`
...
**Dosya**: `docker-compose.yml` (ağ bölümü)
...
**Dosya**: `docker-compose.yml` (ciltler bölümü)
...
**Dosya**: `scripts/configure_platform.py`
...
1. **Ayarları Kontrol Et** - Durum kontrollerini çalıştırın
2. **İlk Adımlar** - Bkz. [İlk Adımlar Kılavuzu](first-steps.md)
3. **Airbyte'ı Yapılandırın** - Bkz. [Airbyte Entegrasyonu](../guides/airbyte-integration.md)
4. **Dremio'yu Yapılandırın** - Bkz. [Dremio Yapılandırması](../guides/dremio-setup.md)

---

**Yapılandırma Kılavuzu Sürümü**: 3.2.0  
**Son Güncelleme**: 2025-10-16  
