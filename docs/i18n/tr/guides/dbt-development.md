...
3. [Veri Modelleme](#veri modelleme)
4. [Test Çerçevesi](#test-çerçevesi)
5. [Belgeleme](#belgeleme)
6. [Makrolar ve Paketler](#makrolar ve paketler)
7. [Artımlı Modeller](#artımlı modeller)
8. [Düzenleme İş Akışı](#düzenleme-iş akışı)
...
dbt (veri oluşturma aracı), analitik mühendislerinin SQL ve yazılım mühendisliğinin en iyi uygulamalarını kullanarak depodaki verileri dönüştürmesine olanak tanır. Bu kılavuz, projenin başlatılmasından ileri geliştirme tekniklerine kadar her şeyi kapsar.

### DBT nedir?

dbt, aşağıdakileri kullanarak ham verileri analize hazır veri kümelerine dönüştürür:

- **SQL Dönüşümleri**: SELECT ifadelerini yazın, dbt gerisini halleder
- **Sürüm Kontrolü**: İşbirliği için Git entegrasyonu
- **Test**: Entegre veri kalitesi test çerçevesi
- **Belgeler**: Kökeni içeren, kendi kendine oluşturulan belgeler
- **Modülerlik**: Yeniden kullanılabilir şablonlar ve makrolar
...
### dbt_project.yml'yi yapılandırın
...
Aşamalandırma modelleri, kaynaklardan gelen ham verileri temizler ve standartlaştırır.
...
Ara modeller verileri birleştirir ve zenginleştirir.
...
**Özellik Belgeleri**:
- **Soyun Grafikleri**: Model bağımlılıklarının görsel temsili
- **Sütun Ayrıntıları**: Açıklamalar, türler, testler
- **Kaynak Güncelliği**: Veriler yüklendiğinde
- **Proje Görünümü**: BENİOKU içeriği
- **Arama**: Modelleri, sütunları ve açıklamaları bulun

---

## Makrolar ve Paketler
...
### Yeniden Kullanılabilir SQL Parçacıkları
...
#### Sorun 1: Derleme Hatası

**Hata**: `Compilation Error: Model not found`
...
#### Sorun 2: Döngüsel Bağımlılıklar

**Hata**: `Compilation Error: Circular dependency detected`
...
#### Sorun 3: Başarısız Testler

**Hata**: `ERROR test not_null_stg_customers_email (FAIL 15)`
...
**Hata**: Artımlı model her seferinde sıfırdan yeniden oluşturuluyor
...
Bu eksiksiz dbt geliştirme kılavuzu şunları kapsamaktadır:

- **Proje Yapılandırması**: Başlatma, yapılandırma, ortam yapılandırması
- **Veri Modelleme**: Aşama, ara, gerçek, boyut ve mart modelleri
- **Çerçeve Testleri**: Entegre testler, özel testler, genel testler
- **Belgeler**: Model belgeleri, otomatik olarak oluşturulan site belgeleri
- **Makrolar ve Paketler**: Yeniden kullanılabilir kod, dbt_utils, beklentiler
- **Artımlı Modeller**: Stratejiler ekleme, birleştirme, silme+ekleme
- **İş Akışı Düzenleme**: dbt komutları, ardışık düzen komut dosyaları, Airflow entegrasyonu
- **İyi Uygulamalar**: Adlandırma kuralları, klasör yapısı, belgeler
- **Sorun giderme**: Yaygın sorunlar ve çözümleri

Hatırlanması gereken önemli noktalar:
- SQL SELECT ifadelerini kullanın, dbt DDL/DML'yi yönetir
- Entegre test çerçevesiyle erken ve sık test yapın
- Self servis analitiği için belge modelleri
- Büyük tablolar için artımlı modelleri kullanın
- Tutarlı adlandırma kurallarına uyun
- Ortak özellikler için paketlerden yararlanın
