...
3. [REST API](#api-rest)
4. [Ok Uçuş SQL'i](#arrow-flight-sql)
5. [ODBC/JDBC](#odbcjdbc)
6. [Python istemcisi](#client-python)
7. [Java İstemcisi](#java-client)
8. [API Örnekleri](#dapi-examples)

---

## Genel Bakış

Dremio, veri göl eviyle etkileşim kurmak için çeşitli API'ler sağlar:

| API Türü | Kullanım durumları | Liman | Protokol |
|------------|-----------|------|----------|
| REST API'si | Yönetim, meta veriler | 9047 | HTTP/HTTPS |
| ArrowFlightSQL | Yüksek performanslı sorgular | 32010 | gRPC |
| ODBC | BI aracı bağlantısı | 31010 | ODBC |
| JDBC | Java Uygulamaları | 31010 | JDBC |
...
### Bir kimlik doğrulama belirteci oluşturun

**Bitiş noktası**: `POST /apiv2/login`
...
Belirteçlerin geçerliliği varsayılan olarak 24 saat sonra sona erer. `dremio.conf` ile yapılandırın:
...
#### Katalog öğelerini listele

**Bitiş noktası**: `GET /catalog`
...
#### Yola göre bir katalog öğesi alın

**Bitiş noktası**: `GET /catalog/by-path/{path}`
...
### Sanal Veri Kümeleri (VDS)

#### Sanal veri kümesi oluşturun

**Bitiş noktası**: `POST /catalog`
...
#### Sanal veri kümesini güncelleme

**Bitiş noktası**: `PUT /catalog/{id}`
...
**Bitiş noktası**: `DELETE /catalog/{id}?tag={tag}`
...
#### Bir işin durumunu alın

**Bitiş noktası**: `GET /job/{jobId}`
...
**Bitiş noktası**: `POST /job/{jobId}/cancel`
...
#### Bir yansıma oluşturun

**Bitiş noktası**: `POST /reflections`
...
#### Bir S3 kaynağı ekleyin

**Bitiş noktası**: `PUT /source/{name}`
...
#### Kaynak meta verilerini yenile

**Bitiş noktası**: `POST /source/{name}/refresh`
...
Arrow Flight SQL, yüksek performanslı sorgu yürütme sağlar (ODBC/JDBC'den 20-50 kat daha hızlı).

### PyArrow'lu Python istemcisi
...
#### Örnek: Parametrelerle sorgulama
...
- **Kimlik Doğrulama**: REST API ile belirteç tabanlı kimlik doğrulama
- **REST API**: Katalog, SQL yürütme, işler, yansımalar
- **Arrow Flight SQL**: Yüksek performanslı sorgular (20-50 kat daha hızlı)
- **ODBC/JDBC**: BI aracı bağlantısı
- **Python İstemcisi**: İstemci uygulamasını tamamlayın
- **Java istemcisi**: JDBC örnekleri
- **Pratik örnekler**: Raporlama, dışa aktarma, meta veri keşfi

**Önemli çıkarımlar**:
- Yüksek performanslı veri erişimi için Arrow Flight SQL'i kullanın
- Yönetim ve otomasyon için REST API'yi kullanın
- BI araçlarının entegrasyonu için ODBC/JDBC'yi kullanın
- Her zaman kimlik doğrulama belirteçlerini kullanın
- Daha iyi performans için büyük sorguları toplu olarak işleyin

**İlgili belgeler:**
- [Dremio Kurulum Kılavuzu](../guides/dremio-setup.md)
