...
2. [Lệnh CLI](#cli-commands)
3. [API Python](#api-python)
4. [Tệp siêu dữ liệu](#tệp siêu dữ liệu)
5. [API đám mây dbt](#api-dbt-cloud)
6. [Macro tùy chỉnh](#custom-macro)

---

## Tổng quan

dbt cung cấp ba giao diện chính:

| Giao diện | Trường hợp sử dụng | Truy cập |
|--------------|-------------|-------|
| CLI | Phát triển, CI/CD | Dòng lệnh |
| API Python | Thực thi theo chương trình | Mã Python |
| API đám mây dbt | Dịch vụ được quản lý | API REST |
| Siêu dữ liệu | Hướng nội | Tệp JSON |
...
Chạy các mô hình để chuyển đổi dữ liệu.
...
Chạy thử nghiệm chất lượng dữ liệu.
...
Chạy các mô hình, thử nghiệm, hạt giống và ảnh chụp nhanh cùng nhau.
...
### Lệnh của nhà phát triển

#### dbt biên dịch

Biên dịch các mô hình thành SQL mà không cần chạy chúng.
...
Kiểm tra kết nối và cấu hình cơ sở dữ liệu.
...
Liệt kê các nguồn lực của dự án.
...
Tạo bảng kích thước thay đổi chậm loại 2.
...
Xóa các tập tin và tạo phẩm đã biên dịch.
...
### Trình bao bọc Python hoàn chỉnh
...
Chứa siêu dữ liệu dự án hoàn chỉnh.

**Vị trí**: `target/manifest.json`
...
Chứa kết quả thực hiện của lần thực hiện cuối cùng.

**Vị trí**: `target/run_results.json`
...
Chứa thông tin lược đồ cơ sở dữ liệu.

**Vị trí**: `target/catalog.json`
...
Nếu bạn đang sử dụng dbt Cloud (không áp dụng cho cài đặt cục bộ), API sẽ khả dụng.

**URL cơ sở**: `https://cloud.getdbt.com/api/v2`
...
- **Lệnh CLI**: Tham chiếu đầy đủ cho tất cả các lệnh dbt
- **Python API**: Thực thi theo chương trình với trình bao bọc Python
- **Tệp siêu dữ liệu**:Manifest.json, run_results.json, catalog.json
- **dbt Cloud API**: Kích hoạt công việc (nếu sử dụng dbt Cloud)
- **Macro tùy chỉnh**: Tạo và sử dụng các tính năng tùy chỉnh

**Những điểm chính**:
- Sử dụng CLI để phát triển và làm việc tương tác
- Sử dụng API Python để tự động hóa và điều phối
- Phân tích các tệp siêu dữ liệu để xem xét nội tâm
- Tạo macro tùy chỉnh cho logic có thể tái sử dụng
- Tích hợp với Airflow để lập kế hoạch sản xuất

**Tài liệu liên quan**:
- [hướng dẫn phát triển dbt](../guides/dbt-development.md)
- [Hướng dẫn về chất lượng dữ liệu](../guides/data-quality.md)
