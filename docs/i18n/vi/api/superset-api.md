...
2. [Xác thực](#xác thực)
3. [Trang tổng quan](#dashboards)
4. [Đồ họa](#graphics)
5. [Bộ dữ liệu](#datasets)
6. [Phòng thí nghiệm SQL](#sql-lab)
7. [Bảo mật](#security)
8. [Ví dụ về Python](#python-examples)

---

## Tổng quan

Apache Superset cung cấp API REST để truy cập theo chương trình.

**URL cơ sở**: `http://localhost:8088/api/v1`
...
**Điểm cuối**: `POST /api/v1/security/login`
...
**Điểm cuối**: `POST /api/v1/security/refresh`
...
### Trình trợ giúp xác thực Python
...
### Liệt kê trang tổng quan

**Điểm cuối**: `GET /api/v1/dashboard/`
...
### Nhận trang tổng quan

**Điểm cuối**: `GET /api/v1/dashboard/{id}`
...
**Điểm cuối**: `POST /api/v1/dashboard/`
...
**Điểm cuối**: `PUT /api/v1/dashboard/{id}`
...
**Điểm cuối**: `DELETE /api/v1/dashboard/{id}`
...
**Điểm cuối**: `GET /api/v1/dashboard/export/`
...
### Nhập trang tổng quan

**Điểm cuối**: `POST /api/v1/dashboard/import/`
...
**Điểm cuối**: `POST /api/v1/chart/data`
...
**Điểm cuối**: `GET /api/v1/dataset/{id}`
...
### Thêm số liệu được tính toán

**Điểm cuối**: `POST /api/v1/dataset/{id}/metric`
...
**Điểm cuối**: `POST /api/v1/sqllab/execute/`
...
**Điểm cuối**: `GET /api/v1/sqllab/results/{query_id}`
...
### Mã thông báo của khách

**Điểm cuối**: `POST /api/v1/security/guest_token/`
...
**Điểm cuối**: `GET /api/v1/security/roles/`
...
**Điểm cuối**: `POST /api/v1/security/users/`
...
### Tự động hóa bảng điều khiển hoàn chỉnh
...
### Xuất hàng loạt trang tổng quan
...
- **Xác thực**: Xác thực dựa trên mã thông báo JWT
- **Bảng điều khiển**: Hoạt động CRUD, xuất/nhập
- **Biểu đồ**: Tạo, cập nhật, truy vấn dữ liệu
- **Bộ dữ liệu**: Quản lý bảng/lượt xem, số liệu
- **SQL Lab**: Chạy truy vấn theo chương trình
- **Bảo mật**: Mã thông báo khách, người dùng, vai trò
- **Ví dụ về Python**: Tập lệnh tự động hóa hoàn chỉnh

**Những điểm chính**:
- Sử dụng API để tự động hóa bảng điều khiển
- Mã thông báo khách cho phép tích hợp an toàn
- API SQL Lab cho các truy vấn đặc biệt
- Xuất/nhập để kiểm soát phiên bản
- Tạo bộ dữ liệu với số liệu được tính toán

**Tài liệu liên quan**:
- [Hướng dẫn về bảng điều khiển Superset](../guides/superset-dashboards.md)
