...
2. [Cấu hình dự án](#project-configuration)
3. [Mô hình hóa dữ liệu](#data-modeling)
4. [Khung kiểm tra](#test-framework)
5. [Tài liệu](#tài liệu)
6. [Macro và gói](#macro-and-packages)
7. [Mô hình tăng dần](#mô hình tăng dần)
8. [Quy trình điều phối](#orchestration-workflow)
...
dbt (công cụ xây dựng dữ liệu) cho phép các kỹ sư phân tích chuyển đổi dữ liệu trong kho bằng cách sử dụng SQL và các phương pháp hay nhất về công nghệ phần mềm. Hướng dẫn này bao gồm mọi thứ từ khởi tạo dự án đến các kỹ thuật phát triển nâng cao.

###dbt là gì?

dbt chuyển đổi dữ liệu thô thành bộ dữ liệu sẵn sàng phân tích bằng cách sử dụng:

- **Biến đổi SQL**: Viết câu lệnh SELECT, dbt xử lý phần còn lại
- **Kiểm soát phiên bản**: Tích hợp Git để cộng tác
- **Thử nghiệm**: Khung kiểm tra chất lượng dữ liệu tích hợp
- **Tài liệu**: Tài liệu tự tạo có nguồn gốc
- **Tính mô-đun**: Các mẫu và macro có thể tái sử dụng
...
Mô hình dàn dựng làm sạch và chuẩn hóa dữ liệu thô từ các nguồn.
...
Các mô hình trung gian tham gia và làm phong phú dữ liệu.
...
- **Biểu đồ dòng**: Biểu diễn trực quan các phần phụ thuộc của mô hình
- **Chi tiết cột**: Mô tả, loại, bài kiểm tra
- **Độ mới của nguồn**: Khi dữ liệu đã được tải
- **Chế độ xem dự án**: Nội dung README
- **Tìm kiếm**: Tìm mô hình, cột, mô tả
...
### Đoạn mã SQL có thể tái sử dụng
...
#### Vấn đề 1: Lỗi biên dịch

**Lỗi**: `Compilation Error: Model not found`
...
#### Vấn đề 2: Phụ thuộc vòng tròn

**Lỗi**: `Compilation Error: Circular dependency detected`
...
#### Vấn đề 3: Kiểm thử thất bại

**Lỗi**: `ERROR test not_null_stg_customers_email (FAIL 15)`
...
#### Vấn đề 4: Mô hình tăng dần không hoạt động

**Lỗi**: Mô hình gia tăng được xây dựng lại từ đầu mỗi lần
...
Hướng dẫn phát triển dbt hoàn chỉnh này đã đề cập đến:

- **Cấu hình dự án**: Khởi tạo, cấu hình, cấu hình môi trường
- **Mô hình hóa dữ liệu**: Các mô hình dàn dựng, trung gian, thực tế, thứ nguyên và mart
- **Thử nghiệm khung**: Thử nghiệm tích hợp, thử nghiệm tùy chỉnh, thử nghiệm chung
- **Tài liệu**: Tài liệu mô hình, tài liệu trang web được tạo tự động
- **Macro và gói**: Mã có thể tái sử dụng, dbt_utils, kỳ vọng
- **Mô hình gia tăng**: Chiến lược thêm, hợp nhất, xóa+chèn
- **Điều phối quy trình làm việc**: lệnh dbt, tập lệnh quy trình, tích hợp Luồng khí
- **Các phương pháp hay**: Quy ước đặt tên, cấu trúc thư mục, tài liệu
- **Xử lý sự cố**: Các sự cố thường gặp và giải pháp

Những điểm chính cần nhớ:
- Sử dụng câu lệnh SQL SELECT, dbt quản lý DDL/DML
- Kiểm tra sớm và thường xuyên với khung kiểm tra tích hợp
- Mô hình tài liệu để phân tích tự phục vụ
- Sử dụng mô hình gia tăng cho các bảng lớn
- Thực hiện theo các quy ước đặt tên nhất quán
- Tận dụng các gói cho các tính năng phổ biến
