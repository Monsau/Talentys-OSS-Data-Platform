...
3. [データモデリング](#data-modeling)
4. [テストフレームワーク](#test-framework)
5. [ドキュメント](#documentation)
6. [マクロとパッケージ](#macros-and-packages)
7. [増分モデル](#incremental-models)
8. [オーケストレーション ワークフロー](#orchestration-workflow)
9. [グッドプラクティス](#good-practices)
...
dbt (データ構築ツール) を使用すると、分析エンジニアは SQL とソフトウェア エンジニアリングのベスト プラクティスを使用してウェアハウス内のデータを変換できます。このガイドでは、プロジェクトの初期化から高度な開発テクニックまですべてをカバーします。

### DBTとは何ですか?

dbt は、以下を使用して生データを分析対応のデータセットに変換します。

- **SQL 変換**: SELECT ステートメントを作成し、残りは dbt が処理します。
- **バージョン管理**: コラボレーションのための Git 統合
- **テスト**: 統合されたデータ品質テスト フレームワーク
- **ドキュメント**: リネージを含む自己生成ドキュメント
- **モジュール性**: 再利用可能なテンプレートとマクロ
...
ステージング モデルは、ソースからの生データをクリーンアップして標準化します。
...
**機能ドキュメント**:
- **系統グラフ**: モデルの依存関係を視覚的に表現
- **列の詳細**: 説明、タイプ、テスト
- **ソースの鮮度**: データがロードされたとき
- **プロジェクト ビュー**: README コンテンツ
- **検索**: モデル、列、説明を検索します。
...
**エラー**: `Compilation Error: Model not found`
...
**エラー**: `Compilation Error: Circular dependency detected`
...
**エラー**: `ERROR test not_null_stg_customers_email (FAIL 15)`
...
この完全な DBT 開発ガイドでは、以下について説明しています。

- **プロジェクト構成**: 初期化、構成、環境構成
- **データ モデリング**: ステージング、中間、ファクト、ディメンション、およびマート モデル
- **フレームワーク テスト**: 統合テスト、カスタム テスト、汎用テスト
- **ドキュメント**: モデルのドキュメント、自動生成されたサイトのドキュメント
- **マクロとパッケージ**: 再利用可能なコード、dbt_utils、期待値
- **増分モデル**: 追加、マージ、削除+挿入の戦略
- **ワークフロー オーケストレーション**: dbt コマンド、パイプライン スクリプト、Airflow 統合
- **グッドプラクティス**: 命名規則、フォルダー構造、ドキュメント
- **トラブルシューティング**: 一般的な問題と解決策

覚えておくべき重要なポイント:
- SQL SELECT ステートメントを使用し、dbt が DDL/DML を管理します
- 統合テストフレームワークを使用して、早期かつ頻繁にテストします
- セルフサービス分析用の文書モデル
- 大きなテーブルには増分モデルを使用する
- 一貫した命名規則に従う
- 共通機能のパッケージを活用する
