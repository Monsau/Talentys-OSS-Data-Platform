...
Apache Superset は、プログラムによるアクセスのための REST API を提供します。

**ベース URL**: `http://localhost:8088/api/v1`
...
**エンドポイント**: `POST /api/v1/security/refresh`
...
**エンドポイント**: `GET /api/v1/dashboard/{id}`
...
**エンドポイント**: `PUT /api/v1/dashboard/{id}`
...
**エンドポイント**: `DELETE /api/v1/dashboard/{id}`
...
**エンドポイント**: `GET /api/v1/dashboard/export/`
...
**エンドポイント**: `POST /api/v1/dashboard/import/`
...
**エンドポイント**: `POST /api/v1/chart/data`
...
**エンドポイント**: `POST /api/v1/dataset/{id}/metric`
...
**エンドポイント**: `POST /api/v1/sqllab/execute/`
...
**エンドポイント**: `GET /api/v1/sqllab/results/{query_id}`
...
**エンドポイント**: `POST /api/v1/security/guest_token/`
...
**エンドポイント**: `GET /api/v1/security/roles/`
...
**エンドポイント**: `POST /api/v1/security/users/`
...
- **認証**: JWT トークンに基づく認証
- **ダッシュボード**: CRUD 操作、エクスポート/インポート
- **チャート**: データの作成、更新、クエリ
- **データセット**: テーブル/ビュー、メトリクスの管理
- **SQL ラボ**: プログラムによるクエリの実行
- **セキュリティ**: ゲスト トークン、ユーザー、ロール
- **Python の例**: 完全な自動化スクリプト

**重要なポイント**:
- ダッシュボードの自動化に API を使用する
- ゲスト トークンにより安全な統合が可能になります
- アドホック クエリ用の SQL Lab API
- バージョン管理のためのエクスポート/インポート
- 計算されたメトリクスを使用してデータセットを作成する

**関連ドキュメント**:
- [スーパーセット ダッシュボード ガイド](../guides/superset-dashboards.md)
