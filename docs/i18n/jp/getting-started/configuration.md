...
このガイドでは、Airbyte、Dremio、dbt、Apache Superset、PostgreSQL、MinIO、Elasticsearch を含むすべてのプラットフォーム コンポーネントの構成について説明します。適切な構成により、最適なパフォーマンス、セキュリティ、およびサービス間の統合が保証されます。
...
プロジェクトのルートに `.env` ファイルを作成または編集します。
...
**ファイル**: `config/superset_config.py`
...
**ファイル**: `docker-compose.yml` (ネットワークセクション)
...
**ファイル**: `docker-compose.yml` (ボリュームセクション)
...
**ファイル**: `scripts/configure_platform.py`
...
1. **設定を確認** - ヘルスチェックを実行します
2. **最初のステップ** - [ファースト ステップ ガイド](first-steps.md) を参照してください。
3. **Airbyte の設定** - [Airbyte 統合](../guides/airbyte-integration.md) を参照してください。
4. **Dremio の設定** - [Dremio の設定](../guides/dremio-setup.md) を参照してください。
