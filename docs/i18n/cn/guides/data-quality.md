...
1. [概述](#overview)
2. [数据质量框架](#data-quality-framework)
3. [dbt 测试](#dbt-tests)
4. [远大期望整合](#great-expectations-integration)
5. [数据验证规则](#data-validation-rules)
6. [监控和警报](#monitoring-and-alerts)
7.[数据质量指标](#data-quality-metrics)
8. [补救策略](#remediation-strategies)
...
数据质量对于可靠的分析和决策至关重要。本指南涵盖了确保、监控和提高整个平台数据质量的综合策略。
...
|尺寸|描述 |验证示例 |
|----------|-------------|------------------------|
| **准确度** |数据正确反映现实|电子邮件格式验证 |
| **完整性** |不需要缺失值 | NOT NULL 检查 |
| **一致性** |系统间数据匹配|重要外交关系 |
| **新闻** |数据最新且可在需要时使用 |新鲜度检查 |
| **有效期** |数据符合业务规则|值范围检查 |
| **独特性** |无重复记录 |主键唯一性|
...
- **框架**：优质门、层、架构
- **dbt测试**：综合测试、个性化测试、通用宏测试
- **远大的期望**：具有 50 多种期望类型的高级验证
- **验证规则**：业务逻辑、质量评分、监控表
- **监控**：自动警报、气流集成、仪表板
- **指标**：KPI、趋势分析、质量评分
- **补救**：清洁规则、隔离流程、自动更正
- **良好实践**：尽早测试、使用严重性级别、监控趋势
- **案例研究**：真实示例和解决方案

要记住的要点：
- 在每一层实施质量检查（铜牌→银牌→金牌）
- 使用dbt测试进行结构验证，使用Great Expectations进行统计验证
- 监控时间趋势，而不仅仅是时间点指标
- 自动修复常见和可预测的问题
- 在业务影响之前就质量下降发出警报
- 记录质量规则和补救策略
