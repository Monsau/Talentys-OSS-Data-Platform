...
2. [CLI-opdrachten](#cli-opdrachten)
3. [Python-API](#api-python)
4. [Metadatabestanden](#metadata-bestanden)
5. [dbt Cloud-API](#api-dbt-cloud)
6. [Aangepaste macro's](#custom-macro's)

---

## Overzicht

dbt biedt drie hoofdinterfaces:

| Interface | Gebruiksscenario's | Toegang |
|--------------|------------|-------|
| CLI | Ontwikkeling, CI/CD | Commandoregel |
| Python-API | Programmatische uitvoering | Python-code |
| dbt Cloud-API | Beheerde service | REST-API |
| Metagegevens | Introspectie | JSON-bestanden |
...
Voer de modellen uit om de gegevens te transformeren.
...
Voer modellen, tests, zaden en snapshots samen uit.
...
Documentatie genereren en aanbieden.
...
Compileer modellen in SQL zonder ze uit te voeren.
...
Test de databaseverbinding en configuratie.
...
Laad CSV-bestanden in de database.
...
Maak type 2 langzaam veranderende dimensietabellen.
...
Verwijder gecompileerde bestanden en artefacten.
...
Installeer pakketten vanuit pakketten.yml.
...
Bevat de volledige projectmetagegevens.

**Locatie**: `target/manifest.json`
...
Bevat de uitvoeringsresultaten van de laatste uitvoering.

**Locatie**: `target/run_results.json`
...
Bevat databaseschema-informatie.

**Locatie**: `target/catalog.json`
...
Als u dbt Cloud gebruikt (niet van toepassing voor lokale installatie), is de API beschikbaar.
...
### Maak een aangepaste macro

**Bestand**: `macros/custom_tests.sql`
...
**Bestand**: `models/staging/schema.yml`
...
### Geavanceerde macro met argumenten
...
- **CLI-opdrachten**: volledige referentie voor alle dbt-opdrachten
- **Python API**: Programmatische uitvoering met Python-wrapper
- **Metadatabestanden**: manifest.json, run_results.json, catalog.json
- **dbt Cloud API**: taken activeren (bij gebruik van dbt Cloud)
- **Aangepaste macro's**: aangepaste functies maken en gebruiken

**Belangrijkste punten**:
- Gebruik de CLI voor ontwikkeling en interactief werk
- Gebruik de Python API voor automatisering en orkestratie
- Analyseer metadatabestanden voor introspectie
- Maak aangepaste macro's voor herbruikbare logica
- Integreer met Airflow voor productieplanning

**Gerelateerde documentatie**:
- [dbt-ontwikkelingshandleiding](../guides/dbt-development.md)
- [Handleiding gegevenskwaliteit](../guides/data-quality.md)
