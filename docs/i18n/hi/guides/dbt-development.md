...
1. [अवलोकन](#अवलोकन)
2. [प्रोजेक्ट कॉन्फ़िगरेशन](#प्रोजेक्ट-कॉन्फ़िगरेशन)
3. [डेटा मॉडलिंग](#डेटा-मॉडलिंग)
4. [टेस्ट फ्रेमवर्क](#टेस्ट-फ्रेमवर्क)
5. [दस्तावेज़ीकरण](#दस्तावेज़ीकरण)
6. [मैक्रोज़ और पैकेज](#मैक्रोज़-और-पैकेज)
7. [वृद्धिशील मॉडल](#वृद्धिशील-मॉडल)
8. [ऑर्केस्ट्रेशन वर्कफ़्लो](#ऑर्केस्ट्रेशन-वर्कफ़्लो)
...
डीबीटी (डेटा बिल्ड टूल) एनालिटिक्स इंजीनियरों को एसक्यूएल और सॉफ्टवेयर इंजीनियरिंग सर्वोत्तम प्रथाओं का उपयोग करके वेयरहाउस में डेटा को बदलने की अनुमति देता है। यह मार्गदर्शिका परियोजना आरंभीकरण से लेकर उन्नत विकास तकनीकों तक सब कुछ कवर करती है।

### डीबीटी क्या है?

डीबीटी निम्न का उपयोग करके कच्चे डेटा को एनालिटिक्स-तैयार डेटासेट में बदल देता है:

- **एसक्यूएल ट्रांसफॉर्मेशन**: सेलेक्ट स्टेटमेंट लिखें, डीबीटी बाकी को संभालता है
- **संस्करण नियंत्रण**: सहयोग के लिए गिट एकीकरण
- **परीक्षण**: एकीकृत डेटा गुणवत्ता परीक्षण ढांचा
- **दस्तावेज़ीकरण**: वंशावली के साथ स्व-निर्मित दस्तावेज़ीकरण
- **मॉड्यूलैरिटी**: पुन: प्रयोज्य टेम्पलेट और मैक्रोज़
...
### dbt_project.yml कॉन्फ़िगर करें
...
स्टेजिंग मॉडल स्रोतों से कच्चे डेटा को साफ और मानकीकृत करते हैं।
...
मध्यवर्ती मॉडल डेटा को जोड़ते हैं और समृद्ध करते हैं।
...
**सुविधाएँ दस्तावेज़ीकरण**:
- **वंश रेखांकन**: मॉडल निर्भरता का दृश्य प्रतिनिधित्व
- **कॉलम विवरण**: विवरण, प्रकार, परीक्षण
- **स्रोत ताजगी**: जब डेटा लोड किया गया हो
- **परियोजना दृश्य**: README सामग्री
- **खोज**: मॉडल, कॉलम, विवरण ढूंढें
...
#### समस्या 1: संकलन त्रुटि

**त्रुटि**: `Compilation Error: Model not found`
...
#### समस्या 2: परिपत्र निर्भरताएँ

**त्रुटि**: `Compilation Error: Circular dependency detected`
...
#### समस्या 3: असफल परीक्षण

**त्रुटि**: `ERROR test not_null_stg_customers_email (FAIL 15)`
...
#### समस्या 4: वृद्धिशील मॉडल काम नहीं कर रहा

**त्रुटि**: वृद्धिशील मॉडल हर बार स्क्रैच से पुनर्निर्माण करता है
...
इस संपूर्ण डीबीटी विकास मार्गदर्शिका में निम्नलिखित को शामिल किया गया है:

- **प्रोजेक्ट कॉन्फ़िगरेशन**: आरंभीकरण, कॉन्फ़िगरेशन, पर्यावरण कॉन्फ़िगरेशन
- **डेटा मॉडलिंग**: स्टेजिंग, मध्यवर्ती, तथ्य, आयाम और मार्ट मॉडल
- **फ्रेमवर्क टेस्ट**: एकीकृत परीक्षण, कस्टम परीक्षण, सामान्य परीक्षण
- **दस्तावेज़ीकरण**: मॉडल दस्तावेज़ीकरण, स्वतः उत्पन्न साइट दस्तावेज़
- **मैक्रोज़ और पैकेज**: पुन: प्रयोज्य कोड, dbt_utils, अपेक्षाएँ
- **वृद्धिशील मॉडल**: रणनीतियाँ जोड़ें, मर्ज करें, हटाएँ+सम्मिलित करें
- **वर्कफ़्लो ऑर्केस्ट्रेशन**: डीबीटी कमांड, पाइपलाइन स्क्रिप्ट, एयरफ़्लो एकीकरण
- **अच्छी प्रथाएँ**: नामकरण परंपराएँ, फ़ोल्डर संरचना, दस्तावेज़ीकरण
- **समस्या निवारण**: सामान्य समस्याएं और समाधान

याद रखने योग्य मुख्य बिंदु:
- एसक्यूएल सेलेक्ट स्टेटमेंट का उपयोग करें, डीबीटी डीडीएल/डीएमएल का प्रबंधन करता है
- एकीकृत परीक्षण ढांचे के साथ जल्दी और अक्सर परीक्षण करें
- स्व-सेवा विश्लेषण के लिए दस्तावेज़ मॉडल
- बड़ी तालिकाओं के लिए वृद्धिशील मॉडल का उपयोग करें
- लगातार नामकरण परंपराओं का पालन करें
- सामान्य सुविधाओं के लिए पैकेज का लाभ उठाएं
