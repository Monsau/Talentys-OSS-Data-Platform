...
3. [REST API](#api-rest)
4. [SQL Flight SQL](#arrow-flight-sql)
5. [ODBC/JDBC](#odbcjdbc)
6. [Клиент Python](#client-python)
7. [Java-клиент](#java-client)
8. [Примеры API](#dapi-examples)

---

## Обзор

Dremio предоставляет несколько API для взаимодействия с хранилищем данных:

| Тип API | Варианты использования | Порт | Протокол |
|------------|--------|------|----------|
| ОТДЫХ API | Управление, метаданные | 9047 | HTTP/HTTPS |
| СтрелкаFlightSQL | Высокопроизводительные запросы | 32010 | гРПК |
| ОДБК | Возможность подключения инструментов BI | 31010 | ОДБК |
| JDBC | Java-приложения | 31010 | JDBC |
...
**Конечная точка**: `POST /apiv2/login`
...
Срок действия токенов по умолчанию истекает через 24 часа. Настройте в `dremio.conf`:
...
#### Получить элемент каталога по пути

**Конечная точка**: `GET /catalog/by-path/{path}`
...
### Виртуальные наборы данных (VDS)

#### Создайте виртуальный набор данных

**Конечная точка**: `POST /catalog`
...
#### Обновление виртуального набора данных

**Конечная точка**: `PUT /catalog/{id}`
...
**Конечная точка**: `DELETE /catalog/{id}?tag={tag}`
...
**Конечная точка**: `POST /job/{jobId}/cancel`
...
#### Получение списка отражений

**Конечная точка**: `GET /reflections`
...
**Конечная точка**: `POST /reflections`
...
#### Добавьте источник S3

**Конечная точка**: `PUT /source/{name}`
...
#### Обновить исходные метаданные

**Конечная точка**: `POST /source/{name}/refresh`
...
Arrow Flight SQL обеспечивает высокопроизводительное выполнение запросов (в 20–50 раз быстрее, чем ODBC/JDBC).

### Клиент Python с PyArrow
...
### Пример 1. Автоматизированная отчетность
...
### Пример 3: обнаружение метаданных
...
- **Аутентификация**: аутентификация на основе токенов с помощью REST API.
- **REST API**: каталог, выполнение SQL, задания, отражения.
- **Arrow Flight SQL**: высокопроизводительные запросы (в 20–50 раз быстрее).
- **ODBC/JDBC**: подключение инструментов BI.
- **Клиент Python**: полная реализация клиента.
- **Java-клиент**: примеры JDBC.
- **Практические примеры**: отчеты, экспорт, обнаружение метаданных.

**Основные выводы**:
- Используйте Arrow Flight SQL для высокопроизводительного доступа к данным.
- Используйте REST API для управления и автоматизации.
- Используйте ODBC/JDBC для интеграции инструментов BI.
- Всегда используйте токены аутентификации.
- Обработка больших запросов в пакетном режиме для повышения производительности.

**Сопутствующая документация:**
- [Руководство по установке Dremio](../guides/dremio-setup.md)
