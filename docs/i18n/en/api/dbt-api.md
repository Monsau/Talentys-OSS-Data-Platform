...
dbt provides three main interfaces:

| Interface | Use cases | Access |
|---------------|-------------|-------|
...
Run the models to transform the data.
...
Compile models into SQL without running them.
...
Delete compiled files and artifacts.
...
Contains the complete project metadata.
...
Contains the execution results of the last execution.
...
If you are using dbt Cloud (not applicable for local installation), the API is available.
...
- **Custom Macros**: Create and use custom features

**Key points**:
- Use the CLI for development and interactive work
- Use the Python API for automation and orchestration
- Analyze metadata files for introspection
- Create custom macros for reusable logic
- Integrate with Airflow for production planning
