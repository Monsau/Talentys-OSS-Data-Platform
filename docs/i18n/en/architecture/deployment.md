...
- **Scalability**: Manage 10x growth without architectural changes
- **Security**: Defense in depth with multiple layers of security
- **Maintainability**: Easy updates and configuration management
- **Profitability**: Optimize the use of resources

### Environment Types

| Environment | Objective | Scale | Availability |
|---------------|---------|---------|---------------|
| **Development** | Features development, testing | Single node | <95% |
| **Staging** | Pre-production validation | Multi-node | 95-99% |
| **Production** | Live Data Workloads | Clustered | >99.9% |
| **DR** | Disaster recovery site | Production mirror | Standby |
...
**Use Case**: Local development, tests, demonstrations
...
**Benefits**:
- Simple configuration (docker-compose up)
- Low cost
- Rapid iteration
...
### Topology 2: Docker Swarm Multi-Host
...
Our `docker-compose.yml` standard for local development:
...
### Docker Compose Production overheads
...
- **Secrets Manager**: Identifier management
- **ECR**: Container register
...
- **Secret Manager**: Identifier management
...
**When to use**: Unique components reaching resource limits

| Component | Initial | Scaled | Improvement |
|----------|---------|-----------------|---------|
| Dremio Executor | 8 CPU, 16 GB | 16 CPU, 32 GB | 2x query performance |
| PostgreSQL | 4 CPU, 8 GB | 8 CPU, 16 GB | 2x transaction debit |
| Worker Airbyte | 2 CPU, 4 GB | 4 CPU, 8 GB | 2x sync parallelism |
...
**PostgreSQL**: Use pooling connections (PgBouncer)
```ini
# pgbouncer.ini
[databases]
...
**Key Metrics**:
- Airbyte: Sync success rate, synchronized recordings, sync duration
- Dremio: Number of requests, duration of requests, freshness of reflections
- PostgreSQL: Number of connections, transaction rate, cache hit rate
...
3. Apply WAL logs up to the point of failure
4. Restore object storage from snapshot
5. Restart services in dependencies order
...
1. **Size resources correctly**: Monitor actual usage and adjust limits
2. **Use spot/preemptible instances**: For non-critical workloads
3. **Implement data lifecycle policies**: Move cold data to cheaper storage levels
4. **Plan resource scaling**: Reduce during off-peak hours
5. **Use Reserved Instances**: For basic capacity (40-60% savings)

---

## Summary

This deployment architecture guide covers:
...
- **Disaster Recovery**: Backup strategies, RTO/RPO objectives, recovery procedures

Key points to remember:
- Start simple (single-host) and scale as needed
- Kubernetes offers more flexibility for production
- Implement complete monitoring from day one
